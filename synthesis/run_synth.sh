...
echo "生成综合脚本..."
sed "s|__PDK_LIB_PATH__|$PDK_LIB|g" "$SCRIPT_DIR/synth.ys" > "$SCRIPT_DIR/synth_actual.ys"
...
python3 "$PROJECT_ROOT/scripts/yosys2ieda/fix_netlist.py" \
    "$SCRIPT_DIR/results/picorv32_netlist.v" \
    "$SCRIPT_DIR/results/picorv32_netlist.v"

# 复制 SDC 约束到结果目录
cp "$SCRIPT_DIR/constraints/picorv32.sdc" "$SCRIPT_DIR/results/picorv32.sdc"
